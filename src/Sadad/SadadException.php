...
class SadadException extends PoolPortException
{
    public static $errors = array(
        ...
        1029 => 'آدرس IP پرداخت در محدوده آدرس های معتبر اعالم شده توسط پذيرنده نیست .لطفا مسئول فنی پذيرنده با بانک تماس حاصل فرمايند',
        1030 => 'درس Domain پرداخت در محدوده آدرس های معتبر اعالم شده توسط پذيرنده نیست .لطفا مسئول فنی پذيرنده با بانک تماس حاصل فرمايند',
    ...
    public static $userPaymentErrors = array(
        0 => 'نتیجه تراکنش موفق است',
        -1 => 'نتیجه تراکنش ناموفق است.'
    );

    public static $veridyPaymentErrors = array(
        0 => 'نتیجه تراکنش موفق است',
        -1 => 'پارامترهای ارسالی صحیح نیست و يا تراکنش در سیستم وجود ندارد.',
        101 => 'مهلت ارسال تراکنش به پايان رسیده است'
    );

    public static function sendPayRequestError($errorId)
    {
        $errorId = intval($errorId);

        return new self(@self::$errors[$errorId].' #'.$errorId, $errorId);
    }

    public static function userPaymentError($errorId)
    {
        $errorId = intval($errorId);

        return new self(@self::$userPaymentErrors[$errorId].' #'.$errorId, $errorId);
    }

    public static function verifyPaymentError($errorId)
    {
        $errorId = intval($errorId);

        return new self(@self::$veridyPaymentErrors[$errorId].' #'.$errorId, $errorId);
    }
}
